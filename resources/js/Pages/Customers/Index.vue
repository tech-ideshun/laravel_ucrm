<script setup>
    ...
    import { Head, Link } from '@inertiajs/inertia-vue3';   // import先が一緒だとカンマ区切りでimport出来る
    import FlashMessage from '@/Components/FlashMessage.vue';  
    import Pagination from '@/Components/Pagination.vue'; 
    import { ref } from 'vue';
    import { Inertia } from '@inertiajs/inertia';

    defineProps({   // コントローラーから渡ってきた値をキャッチ
        customers : Object   // 複数のデータ群なので型はArray型、単一だとObject型です
    })

    // const searchは検索機能で入力された値が入ってくる受け皿（input側では【v-model="search"】で紐づいてます。）
    const search = ref('')

    // formのボタンが【@click="searchCustomers"】を発火させる方向になってます。発火されたら、上記searchの値を渡してget通信してます。
    // const searchは【ref('')】で用意されているので、値を取得するには【.value】が必要です。
